...
export function registerSTT(api, kv, complete, prompts, opts, logger) {
  ...
  const STT_PROMPT_PATH = new URL("../prompts/stt.md", import.meta.url);
  function loadDefaultPrompt(url, name) {
    try {
      return fs.readFileSync(url, "utf-8").trim();
    } catch {
      logger?.log?.("STT", `Failed to load bundled ${name} prompt`, "error");
      return null;
    }
  }

  const systemPrompt = prompts?.stt || loadDefaultPrompt(STT_PROMPT_PATH, "stt");
